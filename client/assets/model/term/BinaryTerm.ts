...
export default class BinaryTerm extends Term {
	...
	public eval(variableAssignment: Map<string, boolean>): boolean {
		return this.operator.evaluate(this.leftChild!.eval(variableAssignment), this.rightChild!.eval(variableAssignment));
	}

	public toString(): string {
		return "(" + this.leftChild?.toString() + this.operator.getUnifiedSymbol() + this.rightChild?.toString() + ")";
	}
}
